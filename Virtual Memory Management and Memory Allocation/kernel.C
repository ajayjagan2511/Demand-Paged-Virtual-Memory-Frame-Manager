...
void CustomTests(VMPool* pool_a, VMPool* pool_b);
...
#define _TEST_PAGE_TABLE_
	...
	// Console::puts("I am starting with some custom tests of the VM Pool memory allocator.\n");
	// CustomTests(&code_pool, &heap_pool);
...
void CustomTests(VMPool* pool_a, VMPool* pool_b)
{
	...
	if (!pool_a->is_legitimate((unsigned long)probe)) {
		Console::puts("Error: primary::is_legitimate returned false for its own allocation!\n");
	}
	if (pool_b && pool_b->is_legitimate((unsigned long)probe)) {
