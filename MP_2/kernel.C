...
void test_process_pool(ContFramePool * _pool);
...
int main() {
    ...
    //TEST: Comment out the declaration above and the test_memory call below 
    //and then uncomment the following test to see what happens when the info frame is not at 0 and mark_inaccessible is called

    // Test info_frame with more than one frame, not at 0, within pool, and mark_inaccessible
...
// In later machine problems, we will be using two pools. You may want to comment this out and test 
    ...
    //uncomment to test two pools and linked list in release_frames
    // test_process_pool(&process_mem_pool);
...
void test_process_pool(ContFramePool * _pool) {
    // Test process pool allocation and release to verify linked list in release_frames
    unsigned long frame = _pool->get_frames(5);
    _pool->print_frame_states("After allocating 5 frames from process pool");
    
    ContFramePool::release_frames(frame);
    _pool->print_frame_states("After releasing the 5 frames from process pool");
}
